...
def collect_sat_image_layers(row, 
                            sat_image_catalog, 
                            image_size=128, 
                            verbose=False, 
    ...
    date_range = get_date_range(row.date, time_buffer_days=days)
    ...
    items_sentinel = item_details[item_details.platform.str.contains("Sentinel")].sort_values(by="datetime", ascending=False)
    items_landsat  = item_details[item_details.platform.str.contains("landsat")].sort_values(by="datetime", ascending=False)
    if verbose:
    ...
    if num_sentinel > 0:
        for item in items_sentinel.item_obj:
            # If sentinel is available, it is first choice.
            if verbose: 
                print('Searching through Sentinel items')
            ...
            if check_for_water:
                # check if water is visible (NDWI mask must have at least 1 True entry)
                mask, _ = get_ndwi_mask(image_layers, ndwi_limit=limit)
                # if water is found stop searching
                if mask.sum():
                    return image_layers, 'sentinel'
            else:
                # return first found image directly (which is the latest by date)
                return image_layers, 'sentinel'

    if num_landsat > 0:
        for item in items_landsat.item_obj:
            if verbose: 
                print('Searching through Landsat items')
            ...
            if check_for_water:
                ...
                # if water is found stop searching
                if mask.sum():
                    return image_layers, 'landsat'
            else:
                # return first found image directly (which is the latest by date)
                return image_layers, 'landsat'

    # if no images are found use array of zeros when no image is found
    image_layers = dict.fromkeys(BAND_NAMES, np.array(0))
    if verbose:
